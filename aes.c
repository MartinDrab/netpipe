...
{
	rijndaelSetupEncrypt(Ctx->RoundKeys, Key, KeyBits);
	Ctx->Rounds = NROUNDS(KeyBits);
...
{
	rijndaelSetupDecrypt(Ctx->RoundKeys, Key, KeyBits);
	Ctx->Rounds = NROUNDS(KeyBits);
...
{
	const size_t blockCount = Length / 0x10;

	for (size_t i = 0; i < blockCount; ++i) {
		rijndaelEncrypt(Ctx->RoundKeys, Ctx->Rounds, Pt, Ct);
		Pt += 0x10;
		Ct += 0x10;
	}
...
void AES_Decrypt(const AES_Ctx *Ctx, const unsigned char *Ct, size_t Length, unsigned char *Pt)
{
	...
	for (size_t i = 0; i < blockCount; ++i) {
		rijndaelDecrypt(Ctx->RoundKeys, Ctx->Rounds, Ct, Pt);
		Pt += 16;
		Ct += 16;
	}
...
static void _XOR16B(unsigned char Target[16], const unsigned char Source[16])
{
...
{
	unsigned char oneBlock[16];
	const size_t blockCount = Length / 16;

	memcpy(oneBlock, Pt, sizeof(oneBlock));
	_XOR16B(oneBlock, Ctx->IV);
	rijndaelEncrypt(Ctx->RoundKeys, Ctx->Rounds, oneBlock, Ct);
	Pt += 0x10;
	Ct += 0x10;
	for (size_t i = 1; i < blockCount; ++i) {
		memcpy(oneBlock, Pt, sizeof(oneBlock));
		_XOR16B(oneBlock, Ct - 0x10);
		rijndaelEncrypt(Ctx->RoundKeys, Ctx->Rounds, oneBlock, Ct);
		Pt += 0x10;
		Ct += 0x10;
	}
...
void AES_DecryptCBC(const AES_Ctx *Ctx, const unsigned char *Ct, size_t Length, unsigned char *Pt)
{
	...
	for (size_t i = 0; i < blockCount; ++i)
		rijndaelDecrypt(Ctx->RoundKeys, Ctx->Rounds, Ct + i*0x10, Pt + i*0x10);

	_XOR16B(Pt, Ctx->IV);
	for (size_t i = 1; i < blockCount; ++i)
		_XOR16B(Pt + i * 0x10, Ct + (i - 1) * 0x10);
...
void AES_EncryptECB(const AES_Ctx *Ctx, const unsigned char *Pt, size_t Length, unsigned char *Ct)
{
	...
	for (size_t i = 0; i < blockCount; ++i)
		rijndaelEncrypt(Ctx->RoundKeys, Ctx->Rounds, Pt + i * 0x10, Ct + i * 0x10);
...
{
	const size_t blockCount = (Length + 15) / 16;
	const size_t lastLen = (Length % 16) ? (Length % 16) : 16;
	unsigned char buffer[32];
	unsigned char buffer2[32];

	if (blockCount > 2)
		AES_DecryptCBC(Ctx, Ct, (blockCount - 2)*16, Pt);

	Pt += (blockCount - 2) * 16;
	memset(buffer, 0, sizeof(buffer));
	memcpy(buffer, Pt, lastLen + 16);
	AES_Decrypt(Ctx, buffer, 16, buffer2);
	memcpy(buffer, buffer2, 16);
	_XOR16B(buffer, buffer + 16);
	memcpy(buffer + 16 + lastLen, buffer + lastLen, 16 - lastLen);
	AES_Decrypt(Ctx, buffer + 16, 16, buffer2);
	_XOR16B(buffer2, Ct + 16*(blockCount - 2));
	memcpy(Pt, buffer2, 16);
	memcpy(Pt + 16, buffer, lastLen);
